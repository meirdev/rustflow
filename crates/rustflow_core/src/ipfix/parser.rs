...
impl IpfixParser {
    ...
    fn parse_field_value<'a>(
        ...
        match (data_type, length) {
            ...
            (DataType::Unsigned, 3) => {
                map(be_uint(3), |v| FieldValue::Unsigned32(v as u32)).parse(input)
            }
            ...
            (DataType::Signed, 3) => {
                map(be_int(3), |v| FieldValue::Signed32(v as i32)).parse(input)
            }
...
/// Big-endian unsigned integer of a reduced size (RFC 7011 section 6.2).
