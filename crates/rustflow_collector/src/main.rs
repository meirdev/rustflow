...
fn main() {
    ...
    loop {
        ...
        //                     println!("Data Record: {:?}",
        // serde_json::to_string(&data_record).unwrap());                 }
        //             }
        //             netflow_v9::packet::FlowSet::Template(template_flow_set) => {
        //                 for template in &template_flow_set.records {
        //                     println!("Template ID: {}, Fields: {:?}",
        // template.template_id, template.fields);                     
        // parser.register_template(data.header.source_id, template.template_id,
        // netflow_v9::packet::TemplateRecordType::Template(template.clone()));
        //                 }
        //             }
        //             
        ...
        //                     println!("Template ID: {}, Fields: {:?}",
        // template.template_id, template.scope_fields);                     
        // parser.register_template(data.header.source_id, template.template_id,
        // netflow_v9::packet::TemplateRecordType::OptionsTemplate(template.clone()));
        //                 }
        //             }
        //         }
        //     }
                    ...
                    for record in &set.records {
                        match record {
                            ipfix::packet::Record::Template(template_record) => {
                                // println!("Template ID: {}, Fields: {:?}",
                                // template_record.header.template_id, template_record.fields);
                                parser.register_template(
                                    0,
                                    template_record.template_record_header.template_id,
                                    ipfix::packet::TemplateRecordType::Template(
                                        template_record.clone(),
                                    ),
                                );
                            }
                            ipfix::packet::Record::OptionsTemplate(options_template_record) => {
                                // println!("Options Template ID: {}, Fields: {:?}",
                                // options_template_record.header.template_id,
                                // options_template_record.fields);
                                parser.register_template(
                                    0,
                                    options_template_record
                                        .options_template_record_header
                                        .template_id,
                                    ipfix::packet::TemplateRecordType::OptionsTemplate(
                                        options_template_record.clone(),
                                    ),
                                );
                            }
                            ipfix::packet::Record::Data(data_record) => {
                                // println!("Data Record: {:?}",
                                // serde_json::to_string(&data_record).
                                // unwrap());
                            }
                        }
                    }
